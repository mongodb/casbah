...
/*
   "Casbah's DSL in operator" should {
     ...
     "Accept Arrays as values" in {
       val _in = "foo" in Array(1, 8, 12, "x")
       _in must haveListEntry("foo.$in", Array(1, 8, 12, "x"))
     }

     "Accept Tuples of varying arity for values" in {
       "A Tuple4 works" in {
         val _in = "foo" in (1, 8, 12, "x")
         _in must haveListEntry("foo.$in", Array(1, 8, 12, "x"))
       }
       "A Tuple9 works" in {
         val _in = "foo" in (1, 8, 12, "x", "a", "b", "x", 25.2332, BigDecimal("15.32542352"))
         _in must haveListEntry("foo.$in", Array(1, 8, 12, "x", "a", "b", "x", 25.2332, BigDecimal("15.32542352")))
       }
     }
     "Accept Iterables as values" in {
       "An immutable List works" in {
         import scala.collection.immutable.List
         val _in = "foo" in List(1, 8, 12, "x")
         _in must haveListEntry("foo.$in", Array(1, 8, 12, "x"))
       }

       "An immutable Seq works" in {
         import scala.collection.immutable.Seq
         val _in = "foo" in Seq(1, 8, 12, "x")
         _in must haveListEntry("foo.$in", Array(1, 8, 12, "x"))
       }

       "An immutable Set works" in {
         import scala.collection.immutable.Set
         val _in = "foo" in Set(1, 8, 12, "x")
         _in must haveListEntry("foo.$in", Array(1, 8, 12, "x"))
       }

       "An mutable HashSet works" in {
         import scala.collection.mutable.HashSet
         val _in = "foo" in HashSet(1, 8, 12, "x")
         _in must haveListEntry("foo.$in", HashSet(1, 8, 12, "x"))
       }
     }
   }
*/
