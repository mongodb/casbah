...
trait ScalaTestDBObjectMatchers {
  ...
  val beDBObject = Matcher { left: AnyRef =>
    MatchResult(left.isInstanceOf[DBObject], left + " is a DBObject", left + " is not a DBObject")
  }

  def containSomeField(k: String) = Matcher { map: Option[DBObject] =>
  ...
  def containField(k: String) = Matcher { map: DBObject =>
   ...
   */
  def containSomeEntry[V](p: (String, V)) = Matcher { map: Option[DBObject] =>
    MatchResult(
  ...
  def containEntry[V](p: (String, V)) = Matcher { map: DBObject =>
    MatchResult(
  ...
  def containSomeEntries[V](pairs: (String, V)*) = Matcher { map: Option[DBObject] =>
    MatchResult(
   ...
   * can expand dot notation to match specific sub-keys */
  def containEntries[V](pairs: (String, V)*) = Matcher { map: DBObject =>
    MatchResult(
